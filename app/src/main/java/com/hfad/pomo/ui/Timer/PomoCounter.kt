package com.hfad.pomo.ui.Timer
...
import com.hfad.pomo.R

class PomoCounter@JvmOverloads constructor(context: Context, attrSet: AttributeSet? = null, defStyleAttr: Int = 0):
    ...
    var currentPomoPass: Int = 1
        set(value) {
            if (value == 0) {
                totalPomodoros = plannedPomo
            }
            field = value
            postInvalidate()
        }
    ...
    var isTimerRunning: Boolean = false
        set(isRunning) {
            if (isRunning) {
                totalPomodoros = if (currentPomoPass < plannedPomo) {
                    plannedPomo
                } else {
                    currentPomoPass + 1
                }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in 0 until totalPomodoros) {
            ...
            val counterPaint = when {
                i == currentPomoPass -> pendingPomodoroColor
                i >= plannedPomo -> overPlannedPomodoroPaint
                i < currentPomoPass -> passedPomodoroPaint
