...
//@JvmOverloads 는 코틀린 함수의 오버로딩 메소드들을 생성해주는 annotation 입니다.

class TimerView@JvmOverloads constructor(context: Context, attrSet: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrSet, defStyleAttr)
{
    companion object {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val progressAngleInRad = (START_ANGLE + progressAngle) / 180.0 * Math.PI
        val markerX = centerX + radius * Math.cos(progressAngleInRad)
        val markerY = centerY + radius * Math.sin(progressAngleInRad)
